...
final class DebugLocalServer {
    ...
    func login(with request: LoginRequest) async throws -> LoginResponse {
        ...
        if request.role == UserRole.nurse.rawValue {
            ...
            return LoginResponse(status: "success", message: nil, user: .init(username: "nurse", fullName: "Pielęgniarka Demo"))
        }
        if request.username == demoPatientLogin, request.password == demoPatientPassword,
           let patient = snapshot.patients.first {
            return LoginResponse(status: "success", message: nil, user: .init(username: patient.pesel, fullName: patient.fullName))
        }
    ...
    func saveMeasurements(_ payload: MeasurementPayload) async throws {
        ...
        await db.update {
            $0.measurements.append(.init(visitID: payload.visitID, sys: payload.bloodPressureSys, dia: payload.bloodPressureDia, hr: payload.heartRate, glucose: payload.glucoseLevel, notes: payload.notes))
